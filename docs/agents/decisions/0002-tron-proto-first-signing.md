...
3. Map `blockId`/`blockNumber` into `Transaction.blockHeader` (`number` + `parentHash`) so wallet-core derives `ref_block_bytes/hash` from the proto.
4. Enforce `callValue` to be `0` for TRC20 transfers (TransferTRC20Contract does not include callValue).

## Consequences / Notes
- `ref_block_hash` is derived by wallet-core from `BlockHeader` and will not match the `blockId` slice unless a full block header is provided.
